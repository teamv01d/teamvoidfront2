      ...
      <div v-for="i in info" :key="i">
        ...
        <v-layout row class="mt-10">
          <v-flex xs10>
            ...
            <h4 class="not">
              <v-icon>report</v-icon> Yetenek testine girdikten sonra başvurunuz
              tamamlanacaktır.
            </h4>
          </v-flex>
        </v-layout>
...
export default {
  ...
  created() {
    this.$axios.get(endpoint.auth.profile).then((response) => {
